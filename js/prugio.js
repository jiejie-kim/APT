...
const lbImages = [
  ...
  { src: "images/prugio/prugio_view_5.webp", caption: "용인푸르지오 원클러스터" },
  // 투자가치 (5~10)
...
const typeImages = {
  "80A": [
    ...
    { src: "images/prugio/prugio_type_80a_2.png", caption: "80A 확장 기본형" },
    { src: "images/prugio/59a_plan.webp", caption: "80A 유상옵션" },
    { src: "images/prugio/59a_living.webp", caption: "80A 거실" },
    { src: "images/prugio/59a_kitchen.webp", caption: "80A 주방" },
    { src: "images/prugio/59a_room.webp", caption: "80A 방" },
  ],
  "74A": [
...
function pick(card, typeKey) {
  ...
  setImg("tdpImgMain", currentTypeImgs[0]);
  setImg("tdpImgSub1", currentTypeImgs[1]);
  setImg("tdpImgSub2", currentTypeImgs[2]);
  setImg("tdpImgSub3", currentTypeImgs[3]);
  
  // 서브 이미지 클릭 시 메인 교체
  document.querySelectorAll(".tdp-img-sm").forEach((el, i) => {
    el.onclick = () => {
      const img = currentTypeImgs[i + 1];
      if (!img) return;
      setImg("tdpImgMain", img);
      // 메인 이미지 클릭은 라이트박스로
      document.getElementById("tdpImgMain").onclick = () => openTypeLightbox(i + 1);
    };
  });
